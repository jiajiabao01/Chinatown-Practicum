...
From this scatter plot, we see that properties within 500m of I-676 and I-76 follow the expected pattern of increasing property prices as one gets further from the highway (folks want to avoid the noise and air pollution from the highway). The correlation between distance to closest highway (in meters) and Property sale price (log-transformed price per total area) is `r cor_i676` and `r cor_i76` for properties near I-676 and I-76, respectively, meaning for every additional meter away from the highway, property price per square foot of total area increases by `r (round(exp(cor_i676),3) - 1)*100`% and `r (round(exp(cor_i76),3) - 1)*100`%, respectively.

We see a near null effect for property prices and distance to US-1 (correlation of `r cor_us1`), but we see a negative relationship between distance to I-95 and property price (correlation of `r cor_i95`). This means that for every additional meter away from the highway, property price per square foot of total area decreases by `r abs(round(exp(cor_i95),3) - 1)*100`%.
...
```{r i676effects_setup_studyareanew}
# working with the newest philly_properties dataset where data was cleaned for our studyarea
prop_studyarea_0329 <- st_read("Dataset/property_CT.geojson") # 1646 rows

# add features for simple linear regression
props_i676_tomodel <- prop_studyarea_0329 %>% 
  filter() %>% # there are a couple properties with total area < 100; reduces dataset to 990 properties
  # some feature engineering
  mutate(dist_to_I676m = distance_to_I676 * 0.3048, # convert to meters
         log_dist_i676 = log(dist_to_I676m),
...
#### Model Results (for properties in study area)

```{r i676effects_model1_studyarea}
# model with just distance to highway
model_i676 <- lm(log_price ~ dist_to_I676m,
...
```{r i676effects_model2_studyarea}
# model with just distance to highway + zip code
model_i676_zip <- lm(log_price ~ dist_to_I676m + zip_code,
                     data = properties_i676_tomodel)
...
```{r i676effects_model3_studyarea}
# simple model but with more predictors
model_i676_more <- lm(log_price ~ dist_to_I676m + 
                        residential + # internal characteristics
                        year_built + central_air + 
...
```{r i676effects_model3_studyareanew}
# simple model but with more predictors
model_i676_more <- lm(log_price ~ dist_to_I676m + 
                        residential + # internal characteristics
...
discontinuity_plot %>% 
  ...
  ggplot(aes(x = distance_to_highway, y = mean_log_price, color = I676)) +
  geom_point(size = 0.5) +
...
```{r discontinuity_plot_modelline}
