...
I'm continuing to use the prop500 data for this part as we explore what other factors help us explain sale_price.

```{r dist_otheramenities}
# adding distance to other amenities to prop500
prop500_amenities <- prop500 %>% 
  mutate(dist_park = calculate_nearest_distance(geometry,
                                                philly_parks),
         ...
         dist_cityhall = dist_cityhall * 0.3048,
         dist_hospital = dist_hospital * 0.3048)


```

```{r amenities_initial_plots}
# plot to make sure it's doing what i want it to do
# philly parks
ggplot() +
  geom_sf(data = philly_bounds, fill = "#c9b887", color = "grey") +
  geom_sf(data = hydro, fill = "#7d97c7", color = "transparent") +
  geom_sf(data = philly_parks, fill = "#6b9169", color = "transparent") +
  geom_sf(data = prop500_amenities,
          aes(color = dist_park)) +
  labs(title = "Distance to nearest parks") +
  theme_void()
...
ggplot() +
  ...
  geom_sf(data = prop500_amenities,
          aes(color = dist_lib)) +
  geom_sf(data = libraries, color = "coral") +
...
ggplot() +
  ...
  geom_sf(data = philly_parks, fill = "#6b9169", color = "transparent") +
  geom_sf(data = prop500_amenities,
          aes(color = dist_transit)) +
  geom_sf(data = transit, color = "green") +
...
ggplot() +
  ...
  geom_sf(data = prop500_amenities,
          aes(color = dist_lib)) +
  geom_sf(data = school, color = "brown") +
...
ggplot() +
  ...
  geom_sf(data = philly_parks, fill = "#6b9169", color = "transparent") +
  geom_sf(data = prop500_amenities,
          aes(color = dist_cityhall)) +
  geom_sf(data = city_hall, color = "beige") +
...
# dataframe of only features and outcome
features_outcome <- prop500_amenities %>% 
  st_drop_geometry() %>% 
  select(
    # sale_price, log_price, price_perTA, 
         log_price_perTA,
         category_easy, dist_to_closest_highwaym, closest_highway, highway_type,
         dist_parkm, dist_libm, dist_transitm, dist_schoolm, dist_cityhall, dist_hospital)

# correlation matrix
cors_forplot <- features_outcome %>% 
  select_if(is.numeric) %>%
  cor()

# significance test 
sigtest_forplot <- cor.mtest(features_outcome %>% 
  select_if(is.numeric))

# pretty correlation matrix of all continuous variables
png("Output/AmenitiesCorMat.png")
corrplot(cors_forplot, 
         p.mat = sigtest_forplot$p, 
         method = 'circle', 
         type = 'lower', 
         insig='blank',
         addCoef.col ='black', 
...
I want to filter out property data before adding distance to amenities.
